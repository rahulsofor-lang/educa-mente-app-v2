    ...
    return (
      <div className="h-screen flex flex-col items-center justify-center bg-gray-50 p-6 text-center">
        <div className="bg-red-50 p-8 rounded-[40px] border border-red-100 max-w-sm">
          ...
          <p className="text-[10px] font-bold text-gray-400 uppercase leading-relaxed mb-6">Não foi possível sincronizar com o banco de dados. Verifique sua internet e as regras do Firebase.</p>
          <button onClick={() => window.location.reload()} className="btn-premium bg-[#004481] text-white w-full py-4">Tentar Novamente</button>
        </div>
      </div>
    );
  }
      ...
      {!role ? (
        <div className="min-h-full flex flex-col items-center justify-center animate-fade-in py-10">
          ...
          <div className="w-full max-w-xs space-y-4 px-4">
            <button onClick={() => setRole('employee')} className="btn-premium bg-[#004481] text-white w-full py-5 shadow-xl">Sou Colaborador</button>
            <button onClick={() => { setAdminInitialView('login'); setRole('hr'); }} className="btn-premium bg-white border-2 border-[#004481] text-[#004481] w-full py-5">Sou Gestor</button>
            <button onClick={() => setRole('psychologist')} className="btn-premium bg-indigo-900 text-white w-full py-5">Responsável Técnico</button>
            <button onClick={() => { setAdminInitialView('register'); setRole('hr'); }} className="text-[10px] font-black text-[#004481] uppercase tracking-widest w-full text-center mt-4">Registrar Empresa</button>
          </div>
        </div>
      ) : role === 'employee' ? (
        <Survey currentCompany={currentCompany} onLogin={handleLogin} onSubmit={async r => {
          try {
            await db.saveSurveyResponse(r);
          } catch (e) {
            console.error("Falha ao salvar resposta:", e);
          }
        }} onBack={handleLogout} onExitKiosk={() => { setIsKioskMode(false); handleLogout(); }} isKioskMode={isKioskMode} allResponses={responses} />
      ) : role === 'hr' ? (
        <AdminPanel company={currentCompany} allCompanies={companies} responses={responses} onRegister={handleRegister} onUpdateCompany={handleUpdateCompanyData} onResetPassword={async (id, pass) => { 
          await db.updateCompanyData(id, { password: pass });
          setActiveCompanyId(id); 
        }} onLogin={handleLogin} onUpdateSectors={s => activeCompanyId && db.updateCompanyData(activeCompanyId, { sectors: s })} onToggleKiosk={() => setIsKioskMode(!isKioskMode)} isKioskMode={isKioskMode} onLogout={handleLogout} initialView={adminInitialView} />
      ) : (
        !isPsychologistAuthenticated ? (
          ...
          <div className="space-y-6">
            <div className="flex bg-white p-2 rounded-2xl shadow-sm border border-gray-100 mb-6 gap-2 print:hidden">
              <button onClick={() => setPsychologistTab('diagnostic')} className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${psychologistTab === 'diagnostic' ? 'bg-[#004481] text-white shadow-md' : 'text-gray-400 hover:bg-gray-50'}`}>Diagnóstico NR-01</button>
              <button onClick={() => setPsychologistTab('profile')} className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${psychologistTab === 'profile' ? 'bg-[#004481] text-white shadow-md' : 'text-gray-400 hover:bg-gray-50'}`}>Meu Perfil (RT)</button>
              <button onClick={handleLogout} className="px-4 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest text-red-500 hover:bg-red-50">Sair</button>
            </div>
